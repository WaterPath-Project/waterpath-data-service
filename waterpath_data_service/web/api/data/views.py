...
_SCHEMA_EXPANSION: dict[str, list[str]] = {
    "all": ["population", "sanitation", "treatment", "livestock_emissions", "hydrology", "qmra"],
...
async def generate_projection_data(
    ...
    climate_model: str | None = Query(
        ...
        description=(
            ...
            "Use GET /projections/climate-models to list available models."
        ),
        examples=["GFDL-ESM4"],
    ),
):
...
async def download_projection(
    ...
    # Uploaded-case batches do not have a session hydrology folder, so skip
    # hydrology here and only generate the supported non-hydrology projections.
    schemas_to_generate = [schema_name for schema_name in schemas_to_generate if schema_name != "hydrology"]
...
async def generate_input_data_package(session_id: str, gids: str, include_livestock: bool = False, include_hydrology: bool = False, include_qmra: bool = False, climate_model: str | None = Query(None, description="ISIMIP3b climate model name for baseline hydrology inputs (e.g. 'GFDL-ESM4'). Only applies when include_hydrology=true. When omitted, the first available model is used. Use GET /projections/climate-models to list available models.", examples=["GFDL-ESM4"])):
    ...
    if include_qmra:
        _qmra_dir = session_dir / "baseline" / "qmra"
        if (_qmra_dir / "treatment.tif").is_file():
            d["qmra"] = {
                "status": "written",
