...
# Required variable subdirectories inside a model folder. River temperature
# may instead be generated by the WorldClim fallback.
...
_CANONICAL_MONTHLY_FILES: dict[str, tuple[str, ...]] = {
    variable: tuple(f"{variable}_m{month:02d}.tif" for month in range(1, 13))
    for variable in _MODEL_VARIABLE_DIRS
}

_ROUTING_FILES: list[str] = ["flowdir.tif", "flowacc.tif"]

# Native resolution of the bundled global hydrology grids (ISIMIP3b, 0.5°).
_HYDRO_NATIVE_RES_DEG: float = 0.5

# A study area spanning fewer than this many native hydrology cells in either
# dimension is smaller than the data can honestly resolve.  Mirrors the
# livestock GLW4 guard (_MIN_GLW4_PIXELS).  Used only to annotate assumptions;
...
def _generate_river_temperature_fallback(
    ...
    * **BIO6** (band 6) – min temperature of the coldest month (°C).
    ...
    Output files are named ``river_temperature_m{month:02d}.tif`` inside
    ...
    Returns
    -------
    List of written filenames (``["river_temperature_m01.tif", …]``).
    ...
    *A* is derived from the difference between the *extreme* monthly
    ...
    for stale_tif in out_var_dir.glob("*.tif"):
        stale_tif.unlink()
    ...
    for month in range(1, 13):
        ...
        out_fname = f"river_temperature_m{month:02d}.tif"
...
def generate_hydrology_inputs(
    ...
    * ``doc``           – absolute path of ``doc.tif``, or ``None``.
    ...
    # Flag study areas smaller than the native hydrology grid can resolve.
    # bounds = (minx, miny, maxx, maxy) in decimal degrees.
    _minx, _miny, _maxx, _maxy = gdf.total_bounds
    _extent_lon = float(_maxx - _minx)
    _extent_lat = float(_maxy - _miny)
    _cells_lon = _extent_lon / _HYDRO_NATIVE_RES_DEG
    _cells_lat = _extent_lat / _HYDRO_NATIVE_RES_DEG
    _below_native_res = min(_cells_lon, _cells_lat) < _MIN_HYDRO_CELLS
    ...
    for var_dir_name in _MODEL_VARIABLE_DIRS:
        ...
        if not var_src_dir.is_dir():
            if var_dir_name == "river_temperature":
                continue
            raise FileNotFoundError(
                f"Required hydrology directory '{var_dir_name}' not found in "
        ...
        canonical_files = _CANONICAL_MONTHLY_FILES[var_dir_name]
        monthly_sources: list[Path] = []
        for canonical_name in canonical_files:
            source = var_src_dir / canonical_name
            if not source.is_file():
                raise FileNotFoundError(
                    f"Model '{model_dir.name}' has no monthly {var_dir_name} file "
                    f"named '{canonical_name}'."
                )
            monthly_sources.append(source)

        for stale_tif in var_out_dir.glob("*.tif"):
            stale_tif.unlink()
        ...
        if len(clipped_files) != 12:
            raise RuntimeError(
                f"Generated {len(clipped_files)} of 12 required {var_dir_name} files "
                f"from model '{model_dir.name}'."
            )
    ...
    if "river_temperature" not in result["variables"]:
        try:
            ...
            if len(fallback_files) != 12:
                raise RuntimeError(
                    f"Generated {len(fallback_files)} of 12 required "
                    "river_temperature fallback files."
                )
            result["variables"]["river_temperature"] = fallback_files
            result["river_temperature_source"] = "worldclim_fallback"
        except Exception as exc:
            raise RuntimeError(
                "River-temperature WorldClim fallback failed."
            ) from exc
